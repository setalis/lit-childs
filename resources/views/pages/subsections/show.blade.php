...
<div class="flex flex-col items-center justify-center border-b border-yellow-500">
    <div class="container flex flex-col md:flex-row mx-auto lg:px-8 px-4">
        <div class="w-3/4 flex flex-col justify-center">
            <h1 class="text-4xl font-bold mb-4 uppercase text-[#28569A]">
                @if($subsection->parent_id)
                    Підрозділ {{ $subsection->parent->section->order }}.{{ $subsection->parent->order }}.{{ $subsection->order }}
                @else
                    Розділ {{ $subsection->section->order }}.{{ $subsection->order }}
                @endif
            </h1>
...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 pb-8 pt-4">
    ...
    <nav class="mb-16 text-sm text-gray-500" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li class="inline-flex items-center"><span>/</span></li>
            @if($subsection->parent_id)
                {{-- Це під-підрозділ: показуємо розділ → підрозділ → під-підрозділ --}}
                <li class="inline-flex items-center">
                    <a href="{{ route('sections.show', $subsection->parent->section) }}" class="text-[#3A6EA5] hover:underline">
                        {{ $subsection->parent->section->title }}
                    </a>
                </li>
                <li><span>/</span></li>
                <li class="inline-flex items-center">
                    <a href="{{ route('subsections.show', $subsection->parent) }}" class="text-[#3A6EA5] hover:underline">
                        {{ $subsection->parent->title }}
                    </a>
                </li>
                <li><span>/</span></li>
            ...
            @else
                {{-- Звичайний підрозділ --}}
                <li class="inline-flex items-center">
                    <a href="{{ route('sections.show', $subsection->section) }}" class="text-[#3A6EA5] hover:underline">
    ...
    @if($subsection->subSubsections->isNotEmpty())
        {{-- Якщо є під-підрозділи — показуємо навігаційні картки до них --}}
        <h2 class="text-2xl font-bold mb-6 text-[#28569A]">Під-підрозділи</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            @foreach($subsection->subSubsections as $subSub)
                <div class="bg-white rounded-xl border-yellow-400 border hover:shadow-lg transition-shadow duration-300">
                    <div class="p-6 flex flex-col justify-between w-full h-full">
                        <div class="mb-4">
                            <span class="text-xs font-semibold text-[#28569A] uppercase tracking-wide">
                                {{ $subsection->section->order }}.{{ $subsection->order }}.{{ $subSub->order }}
                            </span>
                            <h3 class="text-lg font-semibold text-gray-900 mt-1">
                                <a href="{{ route('subsections.show', $subSub) }}" class="hover:text-[#28569A] transition-colors block">
                                    {{ $subSub->title }}
                                </a>
                            </h3>
                        </div>
                        ...
                        <div class="space-y-2 flex-grow">
                            <div class="flex items-center {{ $subSub->theoryBlock ? 'text-gray-600' : 'text-gray-400 opacity-50' }}">
                                <svg class="w-4 h-4 mr-2 border border-yellow-300 rounded p-0.5 flex-shrink-0" fill="#FFBB00" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                                </svg>
                                <span class="text-sm">Теоретичний матеріал</span>
                            </div>
                            <div class="flex items-center {{ $subSub->practiceBlocks->isNotEmpty() ? 'text-gray-600' : 'text-gray-400 opacity-50' }}">
                                <svg class="w-4 h-4 mr-2 border border-yellow-300 rounded p-0.5 flex-shrink-0" fill="#FFBB00" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                                </svg>
                                <span class="text-sm">Практичні завдання</span>
                            </div>
                            <div class="flex items-center {{ $subSub->homeworkBlock ? 'text-gray-600' : 'text-gray-400 opacity-50' }}">
                                <svg class="w-4 h-4 mr-2 border border-yellow-300 rounded p-0.5 flex-shrink-0" fill="#FFBB00" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                                </svg>
                                <span class="text-sm">Самостійна робота</span>
                            </div>
                            <div class="flex items-center {{ $subSub->controlBlocks->isNotEmpty() ? 'text-gray-600' : 'text-gray-400 opacity-50' }}">
                                <svg class="w-4 h-4 mr-2 border border-yellow-300 rounded p-0.5 flex-shrink-0" fill="#FFBB00" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                                </svg>
                                <span class="text-sm">Засоби перевірки</span>
                            </div>
                        </div>

                        <div class="mt-4 pt-4 border-t border-gray-200">
                            <a href="{{ route('subsections.show', $subSub) }}" class="inline-flex items-center text-[#28569A] hover:text-blue-800 font-medium text-sm">
                                Відкрити
    ...
    @else
        {{-- Немає під-підрозділів — показуємо блоки контенту як зазвичай --}}
    ...
    <div class="mt-8 flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-4">
        @if($subsection->parent_id)
            <a href="{{ route('subsections.show', $subsection->parent) }}" class="w-full sm:w-auto border font-semibold border-yellow-500 bg-yellow-500 px-4 py-3 rounded-full">
